...
constexpr char ALPHABET[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmn"
                            "opqrstuvwxyz!#$%&()*+-;<=>?@^_`{|}~";
...
void append_encoded_block(std::vector<std::uint8_t> &out, std::uint32_t value,
...
base85::encode(const std::vector<std::uint8_t> &bytes)
{
    std::vector<std::uint8_t> result;
    result.reserve((bytes.size() + RAW_BLOCK_SIZE - 1) / RAW_BLOCK_SIZE *
                   ENCODED_BLOCK_SIZE);

    for (std::size_t offset = 0; offset < bytes.size();
            offset += RAW_BLOCK_SIZE)
    {
...
std::vector<std::uint8_t>
base85::decode(const std::vector<std::uint8_t> &b85str)
{
    std::vector<std::uint8_t> result;
    result.reserve(b85str.size() / ENCODED_BLOCK_SIZE * RAW_BLOCK_SIZE);
    const auto &table = decode_table();
    for (std::size_t offset = 0; offset < b85str.size();
